...
const getDocumentTypeList = async () => {
  try {
    const res = await selectOptions([1,2]); // 调用新的接口
...
const handleTypeChange = (value: string) => {
  const selectedOption = categorySelect.value.find(
...
const open = async (id: number) => {
  selectedId.value = id; // 接收父组件传递的用户 ID
...
watch(visible, (newVal) => {
  console.log("DocumentUpload visible 值变化:", newVal);
});
