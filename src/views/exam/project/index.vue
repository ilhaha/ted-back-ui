  ...
  <div class="gi_table_page">
    <GiTable title="项目管理" row-key="id" :data="dataList" :columns="columns" :loading="loading"
      ...
      <template #projectType="{ record }">
        <a-tag :color="getExamTypeColor(record.projectType)" bordered>{{
          getExamTypeText(record.projectType)
        }}</a-tag>
      </template>
      ...
      <template #toolbar-left>
        <div class="search-container">
          <a-input v-model="queryForm.projectName" placeholder="搜索项目名称" allow-clear style="width: 100%" />
          <a-input v-model="queryForm.projectCode" placeholder="搜索项目代号" allow-clear
