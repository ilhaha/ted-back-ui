<template>
  <a-modal v-model:visible="visible" title="报名情况" :esc-to-close="false" width="60%" draggable :footer="null"
    ...
    <a-spin :loading="loading">
      <!-- 项目汇总卡片 -->
      <a-alert v-if="projectOverview.length > 0" type="info">
        <template #title>
          <span>项目人项汇总：</span>
          <a-tag v-for="item in projectOverview" :key="item.projectId" style="margin-left: 8px;" color="red"
            size="small">
      ...
      <div class="gi_table_page">
        <GiTable title="" row-key="id" :data="dataList" :columns="columns" :loading="loading"
          ...
          <template #toolbar-left>
            <a-select v-model="queryForm.projectId" placeholder="报考项目" allow-clear class="search-input ml-2"
              @change="search">
              <a-option v-for="item in projectOverview" :key="item.projectId" :value="item.projectId">{{
                item.projectCode }}</a-option>
            </a-select>
            <a-select v-model="queryForm.status" placeholder="报考状态" allow-clear class="search-input ml-2"
              @change="search">
...
import { type NoticeProjectApplyResp, getNoticeProjectApplyOverview } from '@/apis/exam/examNotice'
...
defineOptions({ name: 'OverviewRegistrationModal' })
