  ...
  <div class="gi_table_page">
    ...
    >
      ...
      <template #toolbar-right>
        ...
        <a-button
          v-permission="['exam:examIdcard:audit']"
          type="primary"
          @click="onBatchAudit"
      ...
      <template #educationCertificate="{ record }">
        <a-image
          v-if="record.educationCertificate"
          width="80"
          height="80"
          :src="record.educationCertificate"
          :preview="true"
          :preview-props="{
      ...
      <template #action="{ record }">
        <a-space>
          ...
          <a-link
            ...
            v-if="record.educationVerifyStatus == '0'"
            >审核</a-link
          >
          <!-- <a-link
...
import {
  type ExamIdcardResp,
  type ExamIdcardQuery,
  deleteExamIdcard,
  exportExamIdcard,
  listExamIdcard,
  auditExamIdcard,
} from "@/apis/exam/examIdcard";
...
defineOptions({ name: "InspectionExamIdcard" });
...
const {
  ...
  select,
  selectAll,
} = useTable((page) => listExamIdcard({ ...queryForm, ...page }), {
  immediate: true,
});
...
const columns = ref<TableInstanceColumns[]>([
  ...
  {
    title: "学信网学历验证报告",
    dataIndex: "educationCertificate",
    slotName: "educationCertificate",
  },
  {
    title: "认证状态",
    dataIndex: "educationVerifyStatus",
    slotName: "educationVerifyStatus",
  },
  {
    title: "提交时间",
    dataIndex: "educationVerifyTime",
    slotName: "educationVerifyTime",
  },
  {
    title: "驳回备注",
    dataIndex: "educationVerifyRemark",
    slotName: "educationVerifyRemark",
  },
  {
    title: "操作",
    dataIndex: "action",
    ...
    show: has.hasPermOr([
      "exam:examIdcard:detail",
      "exam:examIdcard:update",
      "exam:examIdcard:delete",
      "exam:examIdcard:audit",
    ]),
  },
]);
...
const ExamIdcardDetailDrawerRef =
  ref<InstanceType<typeof ExamIdcardDetailDrawer>>();
// 详情
...
const onBatchAudit = () => {
  const pendingItems = dataList.value.filter(
    (item: ExamIdcardResp) =>
      selectedKeys.value.includes(item.id) && item.educationVerifyStatus == "0"
  );
