...
const SpecialCertificationApplicantAddModalRef =
  ref<InstanceType<typeof SpecialCertificationApplicantAddModal>>();
// 新增
const onAdd = () => {
...
const SpecialCertificationApplicantDetailDrawerRef =
  ref<InstanceType<typeof SpecialCertificationApplicantDetailDrawer>>();
// 详情
...
const handleBatchAudit = async () => {
  const unAuditedRows = dataList.value.filter(
    (row) => selectedKeys.value.includes(row.id) && row.status === 0
  );
