    ...
    >
      <template #action="{ record }">
        <a-space>
          ...
          <!-- 审核按钮 -->
          <a-dropdown v-if="Number(record.status) === 0" v-permission="['training:training:update']">
            <a-link>
              审核
              <icon-down />
            </a-link>
            <template #content>
              <a-doption @click="onApprove(record)">
                <template #icon><icon-check /></template>
                审核通过
              </a-doption>
              <a-doption @click="onReject(record)">
...
import { type TrainingQuery, type TrainingResp, deleteTraining, exportTraining, listTraining, updateTrainingStatus } from '@/apis/training/training'
import type { TableInstanceColumns } from '@/components/GiTable/type'
import { useDownload, useTable } from '@/hooks'
import { isMobile } from '@/utils'
import has from '@/utils/has'
import { selectAnnouncement } from '@/apis/common/announcement'
import message from '@/layout/components/HeaderRightBar/Message.vue'
...
const columns = ref<TableInstanceColumns[]>([
  ...
  { title: '封面', dataIndex: 'coverPath', slotName: 'coverPath' },
...
// 在script部分添加审核方法
const onApprove = async (record: TrainingResp) => {
  try {
    Modal.confirm({
      title: '确认审核通过',
      content: '确定要将此培训审核通过吗？',
      onOk: async () => {
        const res = await updateTrainingStatus(record.id, '1') // 1表示审核通过

        reset() // 调用重置函数，清空搜索条件并刷新数据
      },
    })
  } catch (error) {
...
const onReject = async (record: TrainingResp) => {
  try {
    Modal.confirm({
      title: '确认审核不通过',
      content: '确定要将此培训审核不通过吗？',
      onOk: async () => {
        const res = await updateTrainingStatus(record.id, '2') // 2表示审核不通过
        reset() // 调用重置函数，清空搜索条件并刷新数据
      },
    })
  } catch (error) {
...
const onCancel = async (record: TrainingResp) => {
  try {
    Modal.confirm({
      title: '确认取消审核',
      content: '确定要将此培训取消审核吗？',
      onOk: async () => {
        await updateTrainingStatus(record.id, '0') // 2表示审核不通过
