  ...
  <a-drawer v-model:visible="visible" title="培训内容" :width="width >= 600 ? 600 : '100%'" :footer="false">
    ...
    <div class="header-actions">
      <a-input-search
        ...
        @change="handleSearch"
      />
      <a-button v-permission="['training:chapter:add']" type="primary" @click="addChapter">
    ...
    >
      <template #title="nodeData">
        <span>
          <IconFolder v-if="!nodeData.isLeaf" style="margin-right: 8px; color: #3370ff;" />
          <IconVideoCamera v-else style="margin-right: 8px; color: #3370ff;" />
      ...
      <template #extra="nodeData">
        <template v-if="nodeData.isLeaf">
          ...
          <IconEdit
            v-permission="['training:video:update']"
            style="position: absolute; right: 50px; font-size: 15px; top: 10px; color: #3370ff;" @click="() => updateVideo(nodeData)"
          />

          <IconPlayArrowFill
            v-permission="['training:video:add']"
            style="position: absolute; right: 100px; font-size: 15px; top: 10px; color: #3370ff;" @click="() => playVideo(nodeData)"
          />
        </template>
        ...
        <template v-else>
          ...
          <IconEdit
            v-permission="['training:chapter:update']"
            style="position: absolute; right: 50px; font-size: 15px; top: 10px; color: #3370ff;" @click="() => updateChapter(nodeData)"
          />

          <IconLiveBroadcast
            v-permission="['training:chapter:add']"
            style="position: absolute; right: 100px; font-size: 15px; top: 10px; color: #3370ff;" @click="() => addVideo(nodeData)"
          />

          <IconPlus
            v-permission="['training:chapter:add']"
            style="position: absolute; right: 150px; font-size: 15px; top: 10px; color: #3370ff;" @click="() => addSubChapter(nodeData.id)"
          />
        </template>
      </template>
    </a-tree>
...
import { IconFolder, IconFullscreen, IconFullscreenExit, IconVideoCamera } from '@arco-design/web-vue/es/icon'
import ChapterAddModal from '../chapter/ChapterAddModal.vue'
import VideoAddModal from '../video/VideoAddModal.vue'
import { type TrainingDetailResp, trainingContent } from '@/apis/training/training'
import { type ChapterResp, customizeDeleteChapter } from '@/apis/training/chapter'
import { type VideoResp, customizeaDeleteVideo } from '@/apis/training/video'
