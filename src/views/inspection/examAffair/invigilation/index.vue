    ...
    <GiTable row-key="id" :data="dataList" :columns="columns" :loading="loading"
      ...
      <template #action="{ record }">
        <a-space>
          ...
          <a-link v-permission="['system:user:update']" :disabled="record.isSystem" title="修改"
            @click="onUpdate(record)">
            修改
          </a-link>
          <a-link v-permission="['examAffair:invigilation:delete']" :disabled="record.isSystem" title="重置密码"
            @click="onResetPwd(record)">
            重置密码
          </a-link>
          <a-link v-permission="['system:user:delete']" :disabled="record.isSystem" title="删除"
            @click="onDelete(record)" type="danger">
            删除
          </a-link>
...
const {
  ...
  search,
  handleDelete,
} = useTable((page) => listExamStaffInspection({ ...queryForm, ...page }), {
  immediate: true,
});
