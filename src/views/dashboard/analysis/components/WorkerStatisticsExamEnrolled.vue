<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :title="`作业项目已报名未参考人数统计（总人数：${totalCount}）`">
      <div class="content">
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        if (detailData.value.length > 0) {
          let tableHtml = `
            <table style="border-collapse: collapse; font-size: 12px; table-layout: fixed;">
              ...
              <tr style="background: #f0f0f0;">
                ...
                <th style="padding: 4px 6px; border: 1px solid #ddd; text-align: right;">未参考人数</th>
              </tr>
          `
          detailData.value.forEach((d: any) => {
            tableHtml += `
              <tr>
                ...
                <td style="padding: 4px 6px; border: 1px solid #ddd; text-align: right;">${d.recordCount ?? '-'}</td>
              </tr>
            `
          })
          tableHtml += '</table>'
          ...
          return `
            <div style="min-width: 220px;">
              ...
              <div style="color: #666; margin-bottom: 8px;">未参考人数：${recordCount}</div>
        ...
        return `<div style="min-width: 200px;">
          ...
          <div style="color: #666;">未参考人数：${recordCount}</div>
...
const onChartMouseover = (params: any) => {
  console.log(chartData.value[params.dataIndex]);
