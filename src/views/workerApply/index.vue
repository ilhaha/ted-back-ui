    ...
    >
      ...
      <template #qualificationPath="{ record }">
        ...
        <template v-if="record.qualificationPath">
          <!-- 图片显示缩略图 -->
          <a-image
            v-if="isImage(record.qualificationPath)"
            width="80"
            height="60"
            :src="record.qualificationPath"
            :preview-props="{ zoomRate: 1.5 }"
            class="preview-image"
            fit="cover"
          />
      ...
      <template
        ...
        :key="col.title"
        #[`doc_${col.title}`]="{ record }"
      >
        <span
          v-if="record.docMap && record.docMap[col.title]"
          class="preview-image"
        >
          <template
            v-for="(path, index) in record.docMap[col.title].split(',')"
            :key="index"
          >
            <!-- 图片 -->
            <a-image
...
import {
  type WorkerApplyResp,
  type WorkerApplyQuery,
  listWorkerApply,
  deleteWorkerApply,
} from "@/apis/worker/workerApply";
...
const {
  ...
  pagination,
  search,
} = useTable((page) => listWorkerApply({ ...queryForm, ...page }), {
  immediate: false,
});
...
const baseColumns = ref<TableInstanceColumns[]>([
  ...
  { title: "来源", dataIndex: "applyType", slotName: "applyType" },
  {
    ...
    slotName: "idCardAddress",
  },
  {
    title: "身份证信息",
    dataIndex: "idCardPhotoFront",
    slotName: "idCardPhotoFront",
    width: 200,
    align: "center",
  },
  {
...
watch(
  () => queryForm.status,
  (val) => {
    const remarkIndex = fixedColumns.value.findIndex(
      (c) => c.dataIndex === "remark"
    );
