<template>
  <a-modal v-model:visible="visible" title="题库Excel导出" :mask-closable="false" :width="600" :footer="null">
    <div class="export-modal-content">
        ...
        <a-form layout="vertical">
          <a-form-item label="题目分类" field="categoryIds">
            <a-cascader v-model="selectedCategory" :options="categoryOptions" placeholder="请选择分类"
              :style="{ width: '100%' }" change-on-select />
          </a-form-item>
        </a-form>
      </div>
      ...
      <div class="action-buttons">
        <!-- 下载按钮：添加加载状态 -->
        <a-button type="outline" @click="downloadQuestionsExcel" :loading="isDownloading">
          <template #icon>
            <icon-download />
          </template>
          下载Excel
