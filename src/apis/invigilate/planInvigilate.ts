...
// 修改submitReviewResult参数结构
export const submitReviewResult = (examId: number, data: {
  candidateIds: number[]
  reviewStatus: number
}) => {
  // 将参数转换为后端需要的格式
  const requestBody = data.candidateIds.map((id) => ({
    candidateId: id,
    newStatus: data.reviewStatus,
  }))

  return http.put('/exam/review/status', requestBody, { // 直接发送数组
    params: { examId },
  })
}
